...
**Decision.** Use three layers: `api → service → database`. Shared clients live under `infra/`. Business rules live in `service/`, which also owns all
database queries, writes, and transactions (using `*gorm.DB` directly). The
`model/` layer holds simple data models and seed data with no behavior; we
deliberately do **not** add a repository abstraction over GORM.
...
exists.
- **Repository interfaces (tx-aware methods over GORM)**: would allow mocking the
database in unit tests, but the tx handle (`*gorm.DB`) still leaks through every
method, so the abstraction is not truly swappable. It also splits one logical
transaction across two packages and adds boilerplate. We rely on integration
tests against real Postgres instead, since our correctness depends on real
locks and constraints that a mock cannot reproduce.
